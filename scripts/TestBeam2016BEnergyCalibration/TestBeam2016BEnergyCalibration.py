...
def makePlots(infile,material,e,d):
    iy,idy,iw=fitDir(infile,material+"-"+str(e)+"-"+str(d))
    if iy!=None:
        x.append(d)
        y.append(iy)
        dy.append(idy)
        ...
        myGraph.Draw("ALP")
        c1.Print(material+"-"+str(d)+"-"+str(e)+"-FWHM.pdf")

class exampleAnalysis(AnalysisBase):
    def Init(self):
        return
    
    def analysis(self,filename,myWorker):
        try:
            hSum=ROOT.TH1F("hSum","E_{dep}",300,0,300)
...
if __name__=="__main__":
    ...
    myAnalysis.Init()
    todo=myAnalysis(function=exampleAnalysis.analysis)
    while len(todo):
        todo=myAnalysis(function=analysis)
    infile=ROOT.TFile(self.outfile)
    materials=["G4_GRAPHITE","G4_Al","G4_Mg"]
    ...
    for material in materials:
        for e in energies:
            ...
            dw=[]
            for d in lengths:
                makePlots(infile, material, e, d)
                
    for e in energies:
        makePlots(infile,"G4_GRAPHITE",e,10)       