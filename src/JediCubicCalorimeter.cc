...
#include <JediCubicCalorimeter.hh>
...
JediCubicCalorimeter::JediCubicCalorimeter(std::string infile):JediMainVolume(infile),fHodoscopeShape("pizza") {
	DefineCommands();
}

G4LogicalVolume* JediCubicCalorimeter::MakeCaloCrystal() {
...
G4LogicalVolume* JediCubicCalorimeter::MakeDetector(G4String name, G4Material* mat,G4double width, G4double height, G4double length) {
...
G4LogicalVolume* JediCubicCalorimeter::MakeDeltaECrystal() {
...
G4VPhysicalVolume* JediCubicCalorimeter::Construct() {
	JediMainVolume::Construct();
	if(fInfileName!=""){
...
JediCubicCalorimeter::~JediCubicCalorimeter() {
	// TODO Auto-generated destructor stub
}

void JediCubicCalorimeter::DefineCommands() {
	JediMainVolume::DefineCommands();

	fMessenger->DeclareMethod("update",
			&JediCubicCalorimeter::UpdateGeometry,
			"update geometry");

	auto hodoShapeCmd=fMessenger->DeclareProperty("hodoscope_shape",
			JediCubicCalorimeter::fHodoscopeShape,
			"hodoscope shape");
	hodoShapeCmd.SetCandidates("pizza square");

}

G4double JediCubicCalorimeter::distanceToEdge(G4double a, G4double b,
...
void JediCubicCalorimeter::PlaceCalorimeter(G4LogicalVolume* aDetectorElement) {
	int fx=-999,fy=-999;
...
void JediCubicCalorimeter::PlaceHodoscope(G4LogicalVolume* aDetectorElement) {
	int fx=-999,fy=-999;
	std::stringstream buf;
	buf.clear();
	buf.str(std::string());
